<?php	
/**
 * BaseTemplate class for the WikiNine skin
 *
 * @ingroup Skins
 */
class WikiNineTemplate extends BaseTemplate {
	/**
	 * Outputs the entire contents of the page
	 */
	public function execute() {
		$html = '';
		$html .= Html::rawElement( 'link', ['rel' => 'stylesheet', 'href' => 'https://use.fontawesome.com/releases/v5.0.13/css/all.css' , 'integrity' => 'sha384-DNOHZ68U8hZfKXOrtjWvjxusGo9WQnrNx2sqG0tfsghAvtVlRW3tvkXWZh58N9jp', 'crossorigin' => 'anonymous'] );
		$html .= $this->get( 'headelement' );
		$html .= Html::rawElement( 'div', [ 'id' => 'mw-wrapper' ],
			Html::rawElement( 'div', [ 'class' => $this->isIndexPage() ? 'mw-body index-page' : 'mw-body', 'role' => 'main' ],
				$this->getSiteNotice() .
				$this->getNewTalk() .
				$this->getIndicators() .
				$this->getSearch() .
			...
			Html::rawElement( 'div', [ 'id' => 'mw-navigation' ],
				...
				Html::rawElement( 'div', [ 'class' => 'folder' ], '<div><i class="fas fa-angle-right"></i></div>' ) .
				$this->getFooter()
			)
		);
	...
	protected function isIndexPage() {
		return $this->data['title'] == '首页' || $this->data['title'] == 'index';
	}

	protected function getBodyContent() {
		$html = $this->get( 'bodycontent' );
		// if (!$this->isIndexPage()) return $html;
		// str_replace('<h2>', '<div class="category-block"><h2>', $html);
		return $html;
	}
	...
	protected function getSearch() {
		...
		$html .= Html::rawElement('button', [ 'id' => 'searchGoButton', 'class' => 'searchButton', 'onClick' => '' ],
			Html::rawElement('i', [ 'class' => 'fas fa-search' ])
		);
		// $html .= $this->makeSearchButton( 'go', [ 'id' => 'searchGoButton', 'class' => 'searchButton' ] );
		$html .= Html::closeElement( 'form' );
		
		return Html::rawElement( 'div', ['id' => 'p-search-box' ], $html);
	}

	/**
	 * Generates the sidebar
	 * Set the elements to true to allow them to be part of the sidebar
	 * Or get rid of this entirely, and take the translate(-100%,-50%)specific bits to use wherever you actually want them
